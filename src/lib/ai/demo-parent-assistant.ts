...
export function demoParentAssistantReply(
  ...
  if (
    /врач|доктор|маман|специалист|кого выбра|кого посовет|кімді|ұсына|таңдау керек|рекоменд|пікір|отзыв|choose doctor|лфк үшін/.test(
      text.toLowerCase(),
    )
  ) {
  ...
  return [
    "Мен TanymKids ата-ана көмекшісімін (демо, бұлтсыз).",
    "• пікірлер бойынша маман ұсына аламын;",
    "• терминдерді түсіндірем;",
    "• маманға хабарлама нобайын жазамын.",
    "",
    "Мысалы: «ЛФК үшін кімді таңдау керек?»",
  ].join("\n");
}
